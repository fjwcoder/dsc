...
{
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi' . ' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = \'' . $show_type . '\'' . ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi_2 where oi_2.main_order_id = oi.order_id) = 0 ' . ' AND oi.is_zc_order = 0 ' . $where;
...
if ($action == 'register') {
	...
	if ($bind_type == 1) {
		if (!empty($sess_user)) {
			...
			if (!$connect_user) {
				$connect_serialize = array('openid' => $openid, 'nickname' => $name, 'user_id' => $sess_user);
			...
			$db->query($sql);
			$connect_serialize = array('openid' => str_replace($type . '_', '', $info_user_id), 'nickname' => $name, 'user_id' => $user_info['user_id']);
		...
		$db->autoExecute($ecs->table('users_auth'), $other, 'INSERT');
		$connect_serialize = array('openid' => str_replace($type . '_', '', $info_user_id), 'nickname' => $name, 'user_id' => $other['user_id']);
	...
	if (($action == 'order_list') || ($action == 'order_recycle')) {
		...
		$sql = 'SELECT goods_cause FROM ' . $ecs->table('goods') . ' WHERE goods_id=\'' . $goods_id . '\'';
		$is_cause = $db->getOne($sql);
		$res = array_intersect(explode(',', $is_cause), $cause_list);
