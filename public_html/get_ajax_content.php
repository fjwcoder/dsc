			...
			if (($act == 'get_cat_top_list') && ($tpl == 1)) {
				...
				if (($act == 'get_cat_top_list') && ($tpl == 2)) {
					...
					if (($act == 'get_cat_top_list') && ($tpl == 3)) {
						...
						$result['cat_id'] = $cat_id;
						$file = 'floor_cat_content_' . $cat_id . '_' . $_SESSION['user_rank'];
						...
						if ($goods_list === false) {
							$goods_list = get_floor_ajax_goods($cat_id, $floor_num, $warehouse_id, $area_id, $goods_ids);
						...
						$result = array('error' => 0, 'content' => '');
						$json = new JSON();
						$sql = 'select b.* from ' . $GLOBALS['ecs']->table('brand') . ' as b left join ' . $GLOBALS['ecs']->table('brand_extend') . ' as be on b.brand_id=be.brand_id ORDER BY RAND() LIMIT 17 ';
						$val = '';
