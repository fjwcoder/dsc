        ...
        switch($type) {
            ...
            case Wechat::MSGTYPE_EVENT:
                ...
                if ($wedata['Event'] == Wechat::EVENT_SUBSCRIBE) {
                    ...
                    // 注释 by fjw in 18.4.19: 这个需要好好看看，因为扫码哦~~~~~
    ...
    private function subscribe($openid = '', $scene_id = 0)
    {
        if (!empty($openid)) {
            ...
            if (isset($result['ect_uid'])) {
                ...
                if (empty($users) || empty($result['ect_uid'])) { // 如果users表中不存在信息，或者wechat_user中不存在，则清理wechat_user表
                ...
                if($this->ru_id == 0){
                    ...
                    if ($bonus_num <= 0) {
                        ...
                        if (!empty($bonus_msg)) {
                            ...
                            $this->weObj->sendCustomMessage($msg); // 发送客服消息！！！
                        }
                    }
                }
            } else {
