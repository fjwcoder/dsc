...
class User extends Frontend {
    ...
    public function actionUserCard() {
        ...
        if($generate) {
			...
			$where['scene_id'] = $user_id;
			$where['type'] 		= 1;
			$wechat_qrcode_count = dao('wechat_qrcode')->where($where)->count();
            $qrcodeInfo = $this->weObj->getQRCode($user_id, 0, 2592000);
			$qrcode_url_ticket = 'https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.$qrcodeInfo['ticket'];
			$qrcode_url = $qrcodeInfo['url'];
            $errorCorrectionLevel = 'L'; // 纠错级别：L、M、Q、H
            $matrixPointSize = 8; // 点的大小：1到10
            \Touch\QRcode::png($qrcode_url, $qrcode, $errorCorrectionLevel, $matrixPointSize, 2);
			if ($wechat_qrcode_count > 0) {
				$data = array();
				$data['qrcode_url'] = $qrcodeInfo['url'];
				$data['qrcode_url'] = $qrcode_url_ticket;
				$data['ticket'] 	= $qrcodeInfo['ticket'];
				$data['endtime'] 	= time()+$qrcodeInfo['expire_seconds'];
			...
			if ( strpos($info['headimgurl'],'http://') ===0 || strpos($info['headimgurl'],'https://') ===0 )
			{
				$headimgurl = $info['headimgurl'];
			}else{
				$headimgurl = __HOST__ .$info['headimgurl'];
			}
            // 生成头像
			\Touch\Http::setWay(1);
            $headimg = \Touch\Http::doGet($headimgurl);
			if ($headimg===false){
				\Touch\Http::setWay(3);
				$headimg = \Touch\Http::doGet($headimgurl);
			}
		...
		$this->assign('ewm', __HOST__. '/data/attached/qrcode/drp_' . $user_id . '.png');
