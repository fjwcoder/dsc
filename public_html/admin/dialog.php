...
if ($_REQUEST['act'] == 'dialog_content') {
	...
	if (!empty($external_url)) {
		...
		if (strpos($thumb_url, $http) === false) {
			$thumb_url = $GLOBALS['ecs']->url() . $thumb_url;
		}
	...
	$title = (isset($spec_attr['title']) && ($spec_attr['title'] != ',') ? $spec_attr['title'] : array());
	$subtitle = (isset($spec_attr['subtitle']) && ($spec_attr['subtitle'] != ',') ? $spec_attr['subtitle'] : array());
	...
	$album_list = get_goods_gallery_album(1, $adminru['ru_id'], array('album_id', 'album_mame'), 'ru_id', ' ORDER BY sort_order');
	$smarty->assign('album_list', $album_list);
	...
	$masterTitle = (!empty($_REQUEST['masterTitle']) ? trim($_REQUEST['masterTitle']) : '');
	$_REQUEST['spec_attr'] = !empty($_REQUEST['spec_attr']) ? stripslashes($_REQUEST['spec_attr']) : '';
	...
	if (0 < $cat_id) {
		...
		if (0 < $parent_id['parent_id']) {
			$spec_attr['catChild'] = cat_list($parent_id['parent_id']);
			$spec_attr['Selected'] = $parent_id['parent_id'];
		}
		else {
