	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($account['is_paid'] == 0) {
				$user_info = array();
				if (($is_paid == 1) && (0 < intval($amount['user_id']))) {
					$sql = 'SELECT mobile_phone, email,user_name FROM' . $ecs->table('users') . ' WHERE user_id = \'' . $account['user_id'] . '\' LIMIT 1';
					$user_info = $db->getRow($sql);
				}

				$new_user_account = get_user_surplus($account['user_id']);
				$smsParams = array('user_name' => $user_info['user_name'], 'user_money' => $new_user_account ? $new_user_account : 0, 'op_time' => local_date('Y-m-d H:i:s', gmtime()), 'mobile_phone' => $user_info['mobile_phone'] ? $user_info['mobile_phone'] : '', 'add_time' => local_date('Y-m-d H:i:s', $account['add_time']), 'examine' => '通过', 'fmt_amount' => $amount);
				if (($is_paid == 1) && ($account['process_type'] == 1)) {
					$user_account = get_user_surplus($account['user_id']);
					$fmt_amount = str_replace('-', '', $amount);

					if ($user_account < $fmt_amount) {
						$link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
						sys_msg($_LANG['surplus_amount_error'], 0, $link);
					}
					...
					$smsParams['user_money'] = $new_user_account;
