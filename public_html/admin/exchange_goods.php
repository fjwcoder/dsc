...
if ($_REQUEST['act'] == 'insert') {
	...
	$record = array('goods_id' => intval($_POST['goods_id']), 'exchange_integral' => intval($_POST['exchange_integral']), 'is_exchange' => intval($_POST['is_exchange']), 'is_hot' => intval($_POST['is_hot']), 'is_best' => intval($_POST['is_best']), 'user_id' => $adminru['ru_id'], 'add_time' => gmtime(), 'review_status' => 3);
...
if ($_REQUEST['act'] == 'edit') {
	admin_priv('exchange_goods');
	$sql = 'SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, eg.is_best, g.goods_name, eg.user_id, eg.review_status, eg.review_content ' . ' FROM ' . $ecs->table('exchange_goods') . ' AS eg ' . '  LEFT JOIN ' . $ecs->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' . ' WHERE eg.goods_id=\'' . $_REQUEST['id'] . '\'';
	$goods = $db->GetRow($sql);
	set_default_filter();
	$smarty->assign('goods', $goods);
...
if ($_REQUEST['act'] == 'update') {
	...
	$record = array('goods_id' => $goods_id, 'exchange_integral' => $exchange_integral, 'is_exchange' => $is_exchange, 'is_hot' => $is_hot, 'is_best' => $is_best);
