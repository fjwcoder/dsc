...
if ($_REQUEST['act'] == '') {
	...
	if (($chkGroup == 'all') || ($sessGroup == 'all')) {
		...
		foreach ($action_code as $k => $v) {
			...
			if ($v == 'shop_config') {
				unlink(ROOT_PATH . 'temp/static_caches/shop_config.php');
			}
	...
	$goods['total'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1' . $where_goods);
	$virtual_card['total'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real=0 AND extension_code=\'virtual_card\'' . $where_goods);
	$goods['new'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_new = 1 AND is_real = 1' . $where_goods);
	$virtual_card['new'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_new = 1 AND is_real=0 AND extension_code=\'virtual_card\'' . $where_goods);
	$goods['best'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_best = 1 AND is_real = 1' . $where_goods);
	$virtual_card['best'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_best = 1 AND is_real=0 AND extension_code=\'virtual_card\'' . $where_goods);
	$goods['hot'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_hot = 1 AND is_real = 1' . $where_goods);
	$virtual_card['hot'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND is_hot = 1 AND is_real=0 AND extension_code=\'virtual_card\'' . $where_goods);
	$time = gmtime();
	$goods['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND promote_price>0' . ' AND promote_start_date <= \'' . $time . '\' AND promote_end_date >= \'' . $time . '\' AND is_real = 1' . $where_goods);
	$virtual_card['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete = 0 AND promote_price>0' . ' AND promote_start_date <= \'' . $time . '\' AND promote_end_date >= \'' . $time . '\' AND is_real=0 AND extension_code=\'virtual_card\'' . $where_goods);
	...
	$smarty->assign('comment_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('comment') . ' WHERE status = 0 AND parent_id = 0' . $where_cmt));
	$smarty->assign('today_comment_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('comment') . ' WHERE parent_id = 0' . $where_cmt . ' AND add_time BETWEEN ' . $today_start . ' AND ' . $today_end));
	$smarty->assign('platform_real_goods_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete= 0 AND user_id = 0 AND is_real = 1'));
	$smarty->assign('platform_virtual_goods_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete= 0 AND user_id = 0 AND is_real = 0'));
	$smarty->assign('merchants_real_goods_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete= 0 AND user_id > 0 AND is_real = 1'));
	$smarty->assign('merchants_virtual_goods_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE is_delete= 0 AND user_id > 0 AND is_real = 0'));
	$smarty->assign('today_user_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE reg_time BETWEEN ' . $today_start . ' AND ' . $today_end));
	$smarty->assign('yesterday_user_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE reg_time BETWEEN ' . ($today_start - (3600 * 24)) . ' AND ' . ($today_end - (3600 * 24))));
	$smarty->assign('month_user_number', $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE reg_time BETWEEN ' . $month_start . ' AND ' . $month_end));
	...
	if (($data === false) || (API_TIME < date('Y-m-d H:i:s', time() - 43200))) {
		...
		$Http = new Http();
		$Http->doPost('http://www.baidu.com/', $httpData);
	...
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('users_real') . ' WHERE review_status = 0';
	...
	if ($ecs->http() == 'https') {
		$Loaction = 'https://tao.1688.com/';
	}
	else {
		$Loaction = 'http://tao.1688.com/';
	}
	...
	if ($ecs->http() == 'https') {
		$Loaction = 'https://huopin.1688.com/';
	}
	else {
		$Loaction = 'http://huopin.1688.com/';
	}
	...
	if ($ecs->http() == 'https') {
		$Loaction = 'https://qing.taobao.com/';
	}
	else {
		$Loaction = 'http://qing.taobao.com/';
	}
