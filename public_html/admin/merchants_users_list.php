...
function steps_users_list()
{
	...
	if ($result === false) {
		...
		if ($filter['check'] == 1) {
			$where = ' AND merchants_audit = \'0\'';
		}
		else if ($filter['check'] == 2) {
			$where = ' AND merchants_audit = \'1\'';
		}
		else if ($filter['check'] == 3) {
			$where = ' AND merchants_audit = \'2\'';
		}
		...
		$sql = 'SELECT mis.* ' . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS mis ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' as u on mis.user_id = u.user_id ' . $ex_where . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
