...
function get_pzd_list()
{
	...
	if ($result === false) {
		...
		if ($shop_name) {
			$sql = 'SELECT ru_id FROM' . $GLOBALS['ecs']->table('seller_shopinfo') . ' WHERE shop_name LIKE \'%' . mysql_like_quote($shop_name) . '%\'';
...
function get_data_list($type = 0)
{
	...
	if ($type != 0) {
		...
		if ($result === false) {
			...
			if ($filter['shop_name']) {
				$sql = 'SELECT ru_id FROM' . $GLOBALS['ecs']->table('seller_shopinfo') . ' WHERE shop_name LIKE \'%' . mysql_like_quote($filter['shop_name']) . '%\'';
