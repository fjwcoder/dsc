...
if ($_REQUEST['act'] == 'get_select_category') {
	...
	$sec_num = (empty($_REQUEST['sec_num']) ? 0 : intval($_REQUEST['sec_num']));
	$sec_price = (empty($_REQUEST['sec_price']) ? 0 : floatval($_REQUEST['sec_price']));
	$sec_limit = (empty($_REQUEST['sec_limit']) ? 0 : intval($_REQUEST['sec_limit']));
	$result = array('error' => 0, 'message' => '', 'content' => '');
	...
	foreach ($fittings as $val) {
		...
		if (!$db->getOne($sql)) {
			$sql = 'INSERT INTO ' . $ecs->table('seckill_goods') . ' (sec_id, tb_id, goods_id, sec_price, sec_num, sec_limit) ' . 'VALUES (\'' . $sec_id . '\',\'' . $tb_id . '\', \'' . $val . '\', \'' . $sec_price . '\', \'' . $sec_num . '\', \'' . $sec_limit . '\')';
			$db->query($sql);
		}
	}
	...
	array('')
		...
		if (strpos($this_img_info['thumb_url'], $http) === false) {
			$this_img_info['thumb_url'] = $GLOBALS['ecs']->url() . $this_img_info['thumb_url'];
		}
	...
	$result['navColor'] = isset($spec_attr['navColor']) ? trim($spec_attr['navColor']) : 0;
	$count = COUNT($pic_src);
	$arr = array();
	$sort_vals = array();
	...
	$sql = 'SELECT pic_file,pic_thumb,pic_image FROM' . $ecs->table('pic_album') . 'WHERE pic_id = \'' . $pic_id . '\' AND ru_id = \'' . $adminru['ru_id'] . '\'';
	...
	$goods_desc = '';
	$img_src = $GLOBALS['ecs']->url() . $img_src;
	$content = $content . '<p><img src=' . $img_src . ' /></p>';
	...
	if (!empty($title)) {
		foreach ($title as $k => $v) {
			$arr[$k]['title'] = $title[$k];
			$arr[$k]['subtitle'] = $subtitle[$k];

			if ($url[$k]) {
				$arr[$k]['url'] = setRewrite($url[$k]);
			}
			else {
				$arr[$k]['url'] = $url[$k];
			}

			$arr[$k]['original_img'] = $original_img[$k];
			$arr[$k]['homeAdvBg'] = $homeAdvBg[$k];
		}
	}
	...
	if (!empty($result['leftAdv'])) {
		foreach ($result['leftAdv'] as $k => $v) {
	...
	foreach ($result['rightAdv'] as $k => $v) {
	...
	if (0 < $result['cat_id']) {
		$cat_info = get_cat_info($result['cat_id'], array('cat_id', 'cat_name', 'cat_alias_name'));
	...
	if ($spec_attr['brand_ids']) {
		...
		$where .= ' AND b.brand_id in (' . $brandId . ')';
		$sql = 'SELECT b.brand_id,b.brand_name,b.brand_logo FROM ' . $GLOBALS['ecs']->table('brand') . ' as b left join ' . $GLOBALS['ecs']->table('brand_extend') . ' AS be on b.brand_id=be.brand_id ' . $where . ' LIMIT 17';
		$brand_list = $db->getAll($sql);
