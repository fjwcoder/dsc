...
if ($_REQUEST['step'] == 'add_cart_combo_goodsAttr') {
	...
	if ($_REQUEST['act'] == 'balance') {
		...
		if ($order_amount <= $user_money) {
			...
			if (0 < $order_info['user_id']) {
				...
				$sql = 'SELECT store_id,pick_code,order_id FROM ' . $GLOBALS['ecs']->table('store_order') . ' WHERE order_id = \'' . $order_info['order_id'] . '\' LIMIT 1';
				...
				if ($user_mobile_phone != '') {
					...
					$store_smsParams = array('user_name' => $user_name, 'order_sn' => $order_info['order_sn'], 'code' => $stores_order['pick_code'], 'store_address' => $store_address, 'mobile_phone' => $user_mobile_phone);
					...
					if ($GLOBALS['_CFG']['sms_type'] == 0) {
						...
						if ((0 < $stores_order['store_id']) && !empty($store_smsParams)) {
							...
							$ali_sms = $GLOBALS['ecs']->ali_yu($store_result);
						}
					}
				}
		...
		if (file_exists(MOBILE_DRP)) {
			...
			$sql = 'INSERT INTO ' . $ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee) ' . ' SELECT \'' . $new_order_id . '\', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee' . ' FROM ' . $ecs->table('cart') . ' WHERE ' . $sess_id . ' AND rec_type = \'' . $flow_type . '\'' . $goodsIn;
			$db->query($sql);
		}
		...
		if (($stores_sms == 1) && (0 < $store_id)) {
			...
			if ($store_id) {
				...
				if (($GLOBALS['_CFG']['sms_type'] == 0) && $user_mobile_phone) {
					...
					if (($GLOBALS['_CFG']['sms_type'] == 1) && $user_mobile_phone) {
						$store_result = sms_ali($store_smsParams, 'store_order_code');
	...
	$json = new JSON();
	$result = array('error' => 0, 'message' => '');
	$rec_id = (!empty($_REQUEST['rec_id']) ? addslashes($_REQUEST['rec_id']) : '');
	$act_sel_id = (!empty($_REQUEST['sel_id']) ? addslashes($_REQUEST['sel_id']) : '');
	$sel_flag = (!empty($_REQUEST['sel_flag']) ? addslashes($_REQUEST['sel_flag']) : '');
	$act_sel = array('act_sel_id' => $act_sel_id, 'act_sel' => $sel_flag);
